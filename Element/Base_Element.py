...
class SoupElement(object):
    ...
    def capture_element(self, element):
        if element.next == 0:
            if element.text == 1:
                element = self.soup.find_all(element.parserLoc[0], text=element.parserLoc[1])[
                    element.num].text
                self.elementParsed = element
                return element
            else:
                element = self.soup.find_all(element.parserLoc[0], class_=element.parserLoc[1])[
                    element.num].text
                return element
        elif element.next == 1:
            if element.class_ == 1:
                element = self.soup.find_all(element.parserLoc[0], text=element.parserLoc[1])[
                    element.num].next_sibing
                self.elementParsed = element
                return element
            else:
                element = self.soup.find_all(element.parserLoc[0], class_=element.parserLoc[1])[
